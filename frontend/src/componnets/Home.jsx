...
function Home() {
  ...
  return (
    ...
    <main className='bg-gradient-to-br from-sky-900 via-slate-600 to-sky-800 py-5  grid-cols-1 md:grid-cols-2 lg:grid-cols-3  px-10 gap-5'>
      ...
      <h1 className='text-3xl text-gray-300  '>All Profiles</h1>
      <Link className='rounded-sm bg-green-500 py-2 px-3 ' to="/addProfile">Add New Profile</Link>
        </div>
      <input type="text" placeholder='Search' onChange={(e) => setProfileSearch(e.target.value)} className='px-5 w-60 rounded-sm bg-white/30 shadow-2xl shadow-gray-900 ' />
    </div>
